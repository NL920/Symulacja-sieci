    ...
    while (std::getline(is, line))
    {
        ...
        switch(parsed.type)
        {
            ...
            case ElementType::LINK: {
                ...
                if (dest_type == "worker") receiver = &(*workers_.find(dest_id)->second);
                else if (dest_type == "store") receiver = &(*storehouses_.find(dest_id)->second);

                if (src_type == "ramp") ramps_.find(src_id)->second->receiver_preferences.add_receiver(receiver);
                else if (src_type == "worker") workers_.find(src_id)->second->receiver_preferences.add_receiver(receiver);
                break;
            }
        }
    }
...
void Factory::save_factory_structure(std::ostream& os) const 
{
    ...
    auto save_links = [&](ElementID src_id, const auto& sender, std::string src_type) {
        for (const auto& [receiver, prob] : sender.receiver_preferences.get_preferences()) {
            std::string dest_type = (dynamic_cast<Worker*>(receiver)) ? "worker" : "store";
            os << "LINK src=" << src_type << "-" << src_id << " dest=" << dest_type << "-" << receiver->get_id() << "\n";
        }
    };
for (const auto& [id, ramp] : ramps_) {
        for (const auto& [receiver, prob] : ramp->receiver_preferences.get_preferences()) {
    ...
    for (const auto& [id, worker] : workers_) {
        for (const auto& [receiver, prob] : worker->receiver_preferences.get_preferences()) {
...
bool Factory::is_consistent() const {
    try {
        for (const auto& [id, ramp] : ramps_) {
            if (ramp->receiver_preferences.get_preferences().empty()) return false;
        }
        for (const auto& [id, worker] : workers_) {
            if (worker->receiver_preferences.get_preferences().empty()) return false;
        }
    } catch (...) {
        return false;
    }
